...
export class GameRoom {
  ...
  aiChooseCard(botIdx) {
    ...
    const wIdx = trickWinner(this.trick, this.trumpSuit, leadSuit);
    const winningTrickIdx = wIdx;
    const winningPlayer = this.trick[winningTrickIdx].playerIdx;
    const winningCard = this.trick[winningTrickIdx].card;
    ...
    if (trumpCards.length > 0) {
      ...
      const higher = trumpCards.filter(c => c.rank > winningCard.rank);
      if (higher.length > 0) {
        higher.sort((a, b) => a.rank - b.rank);
        return higher[0];
      }
  ...
  botPlay(botIdx) {
    ...
    if (this.currentPlayer !== botIdx) return null;
    if (!this.isBot[botIdx]) return null;
    const card = this.aiChooseCard(botIdx);
    if (!card) return null;
    return this.playCard(botIdx, card.id);
  }
...
export function createRoom(creatorName) {
  let code;
  do {
...
export function joinRoom(roomId, socketId, name) {
  ...
  if (!room) return { error: 'اتاق پیدا نشد' };
  if (room.isFull()) return { error: 'اتاق پر است' };
  const seat = room.addPlayer(socketId, name);
  if (seat === -1) return { error: 'اتاق پر است' };
...
export function leaveRoom(socketId) {
  ...
  const room = rooms.get(roomId);
  if (!room) return null;
  const seat = room.removePlayer(socketId);
  playerRoomMap.delete(socketId);
  if (room.isEmpty()) {
    rooms.delete(roomId);
  } else if (room.state !== 'waiting' && seat !== -1) {
    room.replaceWithBot(seat);
  }
  return { room, seat };
}
...
const leaveCooldowns = new Map();

export function setLeaveCooldown(socketId) {
  leaveCooldowns.set(socketId, Date.now());
}

export function isOnLeaveCooldown(socketId) {
  const time = leaveCooldowns.get(socketId);
  if (!time) return false;
  const elapsed = Date.now() - time;
  if (elapsed >= 3 * 60 * 1000) {
    leaveCooldowns.delete(socketId);
    return false;
  }
  return true;
}

export function getLeaveCooldownRemaining(socketId) {
  const time = leaveCooldowns.get(socketId);
  if (!time) return 0;
  const elapsed = Date.now() - time;
