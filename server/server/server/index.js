...
const lastWheelSpin = new Map(); // socketId -> timestamp
...
function runBotTurns(room) {
  ...
  while (room.isBot[room.currentPlayer] && room.state === 'playing' && safety < 20) {
    ...
    if (room.isBot[room.currentPlayer]) {
      setTimeout(() => runBotTurns(room), delay);
      return;
    }
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', (data) => {
    ...
    if (isOnLeaveCooldown(socket.id)) {
      ...
      socket.emit('error', {
        message: `شما ${remaining} ثانیه دیگر نمی‌توانید بازی جدید شروع کنید`
      });
      return;
    }
  ...
  socket.on('startGame', () => {
    ...
    if (room.playerCount < 1) {
      socket.emit('error', { message: 'بازیکن کافی نیست' });
      return;
    }
    room.startGame();
    notifyRoom(room);
  ...
  socket.on('chooseTrump', (suit) => {
    ...
    if (seat !== room.hakem) {
      socket.emit('error', { message: 'شما حاکم نیستید' });
      return;
    }
    if (!SUITS.includes(suit)) {
      socket.emit('error', { message: 'خال نامعتبر' });
      return;
    }
    room.setTrump(suit);
    notifyRoom(room);
  ...
  socket.on('buySubscription', (data) => {
    socket.emit('subscriptionResult', {
      success: true,
      plan: data.plan,
      message: 'اشتراک شما با موفقیت فعال شد (شبیه‌سازی)'
    });
  });
  ...
  socket.on('giftSubscription', (data) => {
    socket.emit('subscriptionResult', {
      ...
      message: `اشتراک برای ${data.recipient} ارسال شد (شبیه‌سازی)`
    });
  });
  ...
  socket.on('spinWheel', () => {
    ...
    if (lastSpin && (now - lastSpin) < 24 * 60 * 60 * 1000) {
      ...
      socket.emit('wheelResult', {
        error: `شما امروز چرخاندید. ${remaining} ساعت دیگر`
      });
      return;
    }
  ...
  function handleLeave(socket) {
    ...
    if (wasPlaying && seat !== -1) {
      setLeaveCooldown(socket.id);
    }
